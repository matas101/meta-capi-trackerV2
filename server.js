...
// fbclid -> _fbc
(function(){const p=new URL(location.href).searchParams;const fbclid=p.get('fbclid');if(fbclid){const v='fb.1.'+Math.floor(Date.now()/1000)+'.'+fbclid;setCookie('_fbc',v)}})();

const urlQ = new URL(location.href).searchParams;
const TEST_EVENT_CODE = urlQ.get('test_event_code') || undefined;

// Visit: Browser+Server mit gleicher event_id
const visitId = (crypto.randomUUID && crypto.randomUUID()) || '${uuidv4()}';
...
app.post("/capi/visit", async (req, res) => {
  try {
    const { event_id, fbp, fbc, artist_name, title, event_source_url, test_event_code } = req.body || {};
...
app.post("/capi/click", async (req, res) => {
  try {
    const { event_id, fbp, fbc, artist_name, title, music_service, event_source_url, to, test_event_code } = req.body || {};
    const payload = buildCapIPayload({
      pixelId: PIXEL_ID,
